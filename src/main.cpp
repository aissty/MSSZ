...
#if defined(NDEBUG)
# error "Msszcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa1552d0338926156ee8705994f61e85112dce2170c3605978413687ca208b761");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Msszcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Msszcoin Signed Message:\n";
...
{
    // Msszcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Msszcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Msszcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Msszcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Msszcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Msszcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfc246eaa6d135cec257983ddca135582b54aa05aa330d30009b37647b9e0a225");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Russians in Britain told to reveal their riches February 3 2018, 12:01am, The Times";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04df946a047d9c9fc9daf5067c9a075fd96cccd878d29f31fd14d3b4cb4fe78fefc3780f12675a77c9cf2eabc5b248dd7a3580cac05d1e840115240b420575d00e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517660759;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088550775;

        if (fTestNet)
        {
            block.nTime    = 1517660649;
            block.nNonce   = 386194395;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x15dfd80c296ad6b2cc4b9312cdcf9844d5def74d79088341bf387ba064a4ef37"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Msszcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MsszcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MsszcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MsszcoinMiner(CWallet *pwallet)
{
    printf("MsszcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("msszcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MsszcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MsszcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MsszcoinMiner, pwallet));
}
